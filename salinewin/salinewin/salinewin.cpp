...
VOID WINAPI sound6() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(t * ((t / 2 >> 10 | t % 16 * t >> 8) & 8 * t >> 12 & 18) | -static_cast<int>(t / 16) + 64);
...
VOID WINAPI sound9() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(t * ((t & 4096 ? t % 65536 < 59392 ? 7 : t & 7 : 16) ^ (1 & t >> 14)) >> (3 & ( -static_cast<int>(t) >> (t & 2048 ? 2 : 10))));
...
int main()
{
	if (MessageBoxW(NULL, L"Run Malware?", L"salinewin.exe by arnikipad", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
